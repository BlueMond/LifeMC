...
public class RemoveCommand extends PluginCommand {
    ...
    public RemoveCommand(LifeMC plugin) {
        super(plugin);
    }
    ...
    public boolean onCommand(CommandSender commandSender, org.bukkit.command.Command command, String s,
        ...
        if (!this.hasPermission(commandSender, "lifemc.lives.remove")) {
            return true;
        }
        ...
        this.getPlugin().getDataHandler().decreaseLives(target.getUniqueId(), targetLives);
